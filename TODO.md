...
| **Empire** | Rendre `Player::$empire` non-nullable | Tranché en session : un joueur d'une partie lancée a **toujours** un empire. Le modèle prétend encore le contraire (`public ?string $empire = null`), et c'est ce `?string` qui a fait planter l'atome `Marker` le jour où le scoreboard s'est mis à le rendre sur chaque ligne. Déjà fait, côté tests : `PlayerBuilder` attribue un empire par défaut et le cas « pas d'empire » a quitté `ScoreBoardTest`. Reste : colonne non-nullable + migration, et le `?? '—'` de `scoreBoard.html.twig`, qui ne couvre plus rien. | Petite | rien |
