...
|:-------:|:--------------------------------------------|:--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:|:-------:|:-----------------------------:|
|  Arb.   | 2 arbitrages produit (smoke E)              |                                                                                                               Preview en allocation incomplète (panier affiché en prix bruts) · gift × option (allocation perdue si carte à option offerte en cadeau)                                                                                                                |  Micro  |          Ta décision          |
|   F2    | Ports & extraction lib (doctrine deux-flux) | ① ElectiveBenefit config-driven — buckets via connecteur, Game\Category sort de Shop/, option: {budget, step} (spécifié, prêt à planifier) · ② ProductInterface/BuyerInterface (fuite cartographiée : `Advance` en dur dans `Product.php`, `PriceCalculator.php`, `LineQuoter.php`, `ProductCatalog.php`, `PromotionEngine.php` ; `Player` [entité Doctrine] en dur dans `OptionCredits.php`, `LineQuoter.php`, `ProductCatalog.php` ; `Category::cases()` itéré dans `PriceCalculator::categoryCredits()` et validé dans `PromotionEngine::giftCandidates()` — vocabulaire cible : facette, pas catégorie ; `ProductInterface` fusionne prix+disponibilité, pas de split Offer séparé) · ③ port Fulfillment (ownAdvances) · ④ port événements (topic Mercure empires/game/{id} — y compris dans le nouveau RejectOrderHandler) · ⑤ traduction d'exceptions type→copie + i18n | Grande  | Ta décision de lancer la lib  |
...
|  Tech.  | Micro-reliquats                             |                                                                                                                 findByGameAndTurn mort · Product::filterByKeys orphelin · REFACTOR-WHEN split moteur en classes d'action (à poser, seuil 4ᵉ famille)                                                                                                                 |  Micro  |             rien              |
| Commit  | Commit unique de revamp                     |                                                                                                                               Squash/reword de WIP + chantier C en un commit final sur refacto/shop, puis merge vers main à ton rythme                                                                                                                               |    —    |              toi              |
